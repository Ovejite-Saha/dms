  ...
  <form method="post" action="<?= base_url() ?>/actions/doc_upload_action.php" enctype="multipart/form-data" id="uploadForm" class="card shadow-sm p-4">
    <div class="row g-3">
      ...
      <div class="col-md-6">
        ...
        <input type="text" name="document_name" id="document_name" class="form-control" required disabled placeholder="Enter document name">
      </div>
      <div class="col-md-6">
...
(function() {
  ...
  function checkReady() {
    btnUpload.disabled = !(ministry.value && division.value && district.value && upazila.value && project.value && docName.value.trim() && fileInput.files.length);
  }

  ministry.addEventListener('change', function() {
    resetSelect(district,'-- Select District --'); resetSelect(upazila,'-- Select Upazila --');
    project.disabled=true; project.value=''; docName.disabled=true; docName.value=''; fileInput.disabled=true; fileInput.value='';
  ...
  project.addEventListener('change', function() {
    docName.disabled=true; docName.value=''; fileInput.disabled=true; fileInput.value='';
    if (this.value) enable(docName);
    checkReady();
  });
  docName.addEventListener('input', function() {
    if (this.value.trim()) enable(fileInput); else { fileInput.disabled=true; fileInput.value=''; }
    checkReady();
  });
