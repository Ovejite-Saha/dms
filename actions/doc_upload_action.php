...
// Fields: Ministry, Division, District, Upazila, Project, Document Name
// All required. Cascading dropdowns handled on frontend.
// ============================================================
...
if ($document_name === '' || !$ministry_id || !$division_id || !$district_id || !$upazila_id || !$project_id) {
    redirect_doc('All fields (Ministry, Division, District, Upazila, Project, Document Name) are required.', 'danger');
}
...
$safeDocument = preg_replace('/[^A-Za-z0-9_-]/', '_', $document_name);
$newFileName  = $safeProject . '_' . $safeDocument . '.' . $ext;

$uploadPath = UPLOAD_DIR . $newFileName;
$counter = 1;
while (file_exists($uploadPath)) {
    $newFileName  = $safeProject . '_' . $safeDocument . '_' . $counter . '.' . $ext;
    $uploadPath   = UPLOAD_DIR . $newFileName;
    $counter++;
}
...
$stmt = $pdo->prepare("
    INSERT INTO documents
        (document_name, ministry_id, division_id, district_id, upazila_id, project_id,
         doc_date, file_path, file_type, uploaded_by, uploader_role)
    VALUES (?,?,?,?,?,?,?,?,?,?,?)
");
$stmt->execute([
    $document_name, $ministry_id, $division_id, $district_id,
    $upazila_id, $project_id, $doc_date, $newFileName, $ext,
    $cu['username'], $cu['role'],
]);
